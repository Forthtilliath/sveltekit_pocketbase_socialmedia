<script lang="ts">
	...
	export let authModel: User | null;

	let contentRef: HTMLElement | undefined;
	$: isAuthor = authModel && post.user.id === authModel.id;
...
<section class="actions">
	<a href="/profiles/{post.user.id}" class="account">
		{#if $pb && post.user.photo}
			<img src={$pb.files.getUrl(post.user, post.user.photo)} alt={post.user.name} />
		{:else}
			<iconify-icon icon="ic:round-account-circle" />
		{/if}
