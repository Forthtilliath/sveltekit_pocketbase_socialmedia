...
<div on:click={() => dispatchEvent('close')} transition:fade={{ duration: 100 }}>
	<section on:click|stopPropagation>
		<header>
			<h1>Change your profile picture</h1>

			<Button variant="ghost" size="icon" on:click={() => dispatchEvent('close')}>
				<iconify-icon icon="iconamoon:close"></iconify-icon>
			</Button>
		</header>

		<form action="/settings?/changeProfilePhoto" method="post" enctype="multipart/form-data">
